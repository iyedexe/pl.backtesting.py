    ...
    if isinstance(trades, pd.DataFrame):
        ...
        if len(trades_df) and strategy_instance:
            for ind in strategy_instance._indicators:
                ...
                ind_name = ind.name  # type: ignore[attr-defined]
                for i, values in enumerate(ind):  # multi-d indicators
                    suffix = f'_{i}' if len(ind) > 1 else ''
                    trades_df[f'Entry_{ind_name}{suffix}'] = values[trades_df['EntryBar'].values]
                    trades_df[f'Exit_{ind_name}{suffix}'] = values[trades_df['ExitBar'].values]
